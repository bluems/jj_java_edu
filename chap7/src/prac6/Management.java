...
class Management {
	static Scanner scanner = new Scanner(System.in); // 어차피 계속 쓰이고 아무 메소드에서나
														...
														// 되므로, Add에서도 써야하므로
														// static
	private static HashMap<String, MemberInfo> MemberList = new HashMap<String, MemberInfo>(); // Add,
																								// View는
																								// 관리모드가
																								// 아니어도
																								// 동작해야
	...
	public void start() throws IOException {
		...
		ManagementLoop: while (true) {
			System.out.println("1. 고객 인적사항 변경 2. 고객 포인트 변경 0. 이전 메뉴");
	...
	private MemberInfo ModifyInfo(String find_name) {
		...
		ModifyInfoLoop: while (true) {
			...
			if (scanner.hasNextInt()) // 정상적인 메뉴 셀렉 확인 후
				...
				case 2:
					System.out.println("수정할 값을 입력해주세요.");
					System.out.println("(" + info.addr + ")");
					System.out.print("=> "); // 어째선가 이곳도 println하면 아래가 문제 발생함.
